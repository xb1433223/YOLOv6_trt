...
if __name__ == "__main__":
    ...
    engine_file_path = "yolov6.engine"

    # load coco labels

    categories = ["Walking", "Pecking", "Inactive", "Dead"]

    # a  YoLov6TRT instance
    yolov6_wrapper = YoLov6TRT(engine_file_path)

    while True:
            ret,Frame=cap.read()
            if ret==True:
                time.sleep(0.00001)
                yolov6_wrapper.infer(Frame)

            else:
                self.cap=open_cam_rtsp(camera_addr,200,1280,720)
                time.sleep(5)
    # destroy the instance
    yolov6_wrapper.destroy()
